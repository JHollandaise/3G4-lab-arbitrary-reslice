  ...
  // Configures the OpenGL drawing context
{
  ...
  if (canvas_type == CANVAS_TL) { // the 3D view window
    ...
    if (!glIsTexture(frame->data_texture)){
      ...
      // This is the first time the GL contslice_index++] ext has been configured - set up the variables that do not depend on the window size or the view angle
  ...
  // Callback function for when the GL canvas is exposed
{
  ...
  case CANVAS_BL:
    ...
    break;
  case CANVAS_BR:
    glDrawPixels(2*MAX_TRAVERSAL, 2*MAX_TRAVERSAL, GL_RGB, GL_UNSIGNED_BYTE, frame->any_rgb);
    break;
  case CANVAS_TL:
    draw_view_window();
    break;
  ...
  // Constructor - loads the data, lays out the canvases and controls using sizers
{
  ...
  theta = 0;
  control_sizer->Add(new wxStaticText(controlwin, wxID_ANY, "theta (degrees)"), 0, wxLEFT, 10);
  control_sizer->Add(new wxSlider(controlwin, THETA_ID, theta, 0, 180, wxDefaultPosition, wxSize(150, -1), wxSL_HORIZONTAL), 0, wxALL, 5);

  phi = 0;
  control_sizer->Add(new wxStaticText(controlwin, wxID_ANY, "phi (degrees)"), 0, wxLEFT, 10);
  control_sizer->Add(new wxSlider(controlwin, PHI_ID, phi, 0, 180, wxDefaultPosition, wxSize(150, -1), wxSL_HORIZONTAL), 0, wxALL, 5);

  psi = 0;
  control_sizer->Add(new wxStaticText(controlwin, wxID_ANY, "psi (degrees)"), 0, wxLEFT, 10);
  control_sizer->Add(new wxSlider(controlwin, PSI_ID, psi, 0, 180, wxDefaultPosition, wxSize(150, -1), wxSL_HORIZONTAL), 0, wxALL, 5);

  // Separate the angle reslice controls from the threshold controls
  control_sizer->AddSpacer(45);
  ...
  // Callback for the theta slider
{
  ...
  theta = (3.14159265359/180)*event.GetPosition();
  construct_any_reslice();
  tl_canvas->Refresh();
  br_canvas->Refresh();
}
...
{
  // convert to radians here
  phi = (3.14159265359/180)*event.GetPosition();
  construct_any_reslice();
  tl_canvas->Refresh();
  br_canvas->Refresh();
}
...
{
  // convert to radians here
  psi = (3.14159265359/180)*event.GetPosition();
  construct_any_reslice();
  tl_canvas->Refresh();
  br_canvas->Refresh();
}
...
bool MyFrame::generate_texture(int image_width, int image_height, int &tex_width, int &tex_height, GLuint texture, unsigned char *image)
...
// Construct the any angle reslice image
{
  ...
  // Bind the new reslice image to the texture map in the view window
  any_texture_valid = generate_texture(2*MAX_TRAVERSAL, 2*MAX_TRAVERSAL, any_tex_width, any_tex_height, any_texture, any_rgb);
}
  ...
  for (g= -MAX_TRAVERSAL ; g<MAX_TRAVERSAL; g++) {
    ...
    for (i=-MAX_TRAVERSAL; i<MAX_TRAVERSAL; i++) {
      ...
      if (x_valid && z_valid && y_valid) {
        ...
        any_rgb[reslice_index++] = MapItoR[0];
        any_rgb[reslice_index++] = MapItoG[0];
        any_rgb[reslice_index++] = MapItoB[0];
      }
    }
  }
